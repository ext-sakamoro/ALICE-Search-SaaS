...
async fn license_handler() -> (HeaderMap, Json<LicenseInfo>) {
    ...
    (h, Json(LicenseInfo {
        ...
        source_code: "https://github.com/ext-sakamoro/ALICE-Search-SaaS".into(),
...
async fn rate_mw(
    ...
    let ok = {
        let mut e = s.rate_limiters.entry(uid).or_insert_with(|| TokenBucket::new(30000.0, 30000.0 / 3600.0));
        e.try_consume()
    };
...
async fn forward(url: &str, req: Request) -> Result<Response, (StatusCode, Json<Err>)> {
    ...
    let body = axum::body::to_bytes(req.into_body(), 5 * 1024 * 1024).await
